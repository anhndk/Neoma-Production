...
(function () {
  ...
  /* ================================================================
     SCROLL-STORY (section 1–6) — kabeh mlebu try/catch. Yen GSAP gagal
     dimuat / error, kelas "no-scrollstory" ditambahke menyang <html>
     lan CSS njupuk alih nampilke kabeh konten statis.

     ------------------------------------------------------------------
     Migrasi saka GSAP ScrollTrigger pin+scrub menyang position:sticky
     (CSS — .sticky-stage ing style.css) + manual scroll-progress (kode
     ing ngisor iki). Alesan: mekanisme ScrollTrigger `pin` (sing
     meksa #sectionN dadi position:fixed sak suwene .pin-spacer-N
     discroll) ngganggu heuristik iOS Safari kanggo auto-hide address
     bar, lan uga sumber "wobble" (kanvas ketarik banjur mantul bali)
     ing Chrome. Kabeh isi animasi (tl.to/tl.set, durasi, ease, stagger,
     label offset kaya "story"/"story+=0.25"/"-=0.5"/"<") ing saben
     buildSectionN() TETEP PADHA PERSIS karo sadurunge — sing diganti
     mung mekanisme sing "muterake" progress-e: mbiyen ScrollTrigger
     scrub sing ngatur timeline.progress() dhewe miturut scroll, saiki
     progress dietung lan diseleh manual dening updateScrollStory() ing
     ngisor, dipicu scroll/resize listener sing di-throttle RAF.
     ================================================================ */

  // Kabeh "track" (pasangan .pin-spacer-N + timeline paused sing kudu
  // di-scrub manual) kadaftar ing kene, siji-sijine lumantar
  // registerScrollStory() ing pungkasan saben buildSectionN().
  const scrollStoryTracks = [];

  function registerScrollStory(trackSelector, timeline, onProgress) {
    const el = document.querySelector(trackSelector);
    if (el) scrollStoryTracks.push({ el, timeline, onProgress: onProgress || null });
  }

  // Siji scroll handler kanggo KABEH section (dudu siji listener per
  // section), niru pemetaan ScrollTrigger start:"top top" ->
  // end:"bottom bottom" nganggo posisi absolut .pin-spacer-N:
  ...
  function updateScrollStory() {
    ...
    for (let i = 0; i < scrollStoryTracks.length; i++) {
      ...
      const trackTop = rect.top + window.scrollY; // posisi absolut top track
      const trackHeight = track.el.offsetHeight;    // tinggi total track
      const scrollable = trackHeight - vh;           // "top top" -> "bottom bottom"
      if (scrollable <= 0) continue;
      const raw = (window.scrollY - trackTop) / scrollable;
      const progress = Math.min(1, Math.max(0, raw));
      track.timeline.progress(progress);
  ...
  // dhewe ing index.html) isa nyeluk iki sawise #cover disembunyikake
  // (display:none), amarga kuwi ngganti dhuwure dokumen lan mula posisi
  // absolut kabeh .pin-spacer-N katut owah — mbiyen dicek nganggo
  // ScrollTrigger.refresh() kanggo alesan sing padha persis.
  window.updateScrollStory = updateScrollStory;
  ...
  function buildSection1() {
    ...
    registerScrollStory("#pin-spacer-1", tl);
  }
  ...
  function buildSection2() {
    ...
    registerScrollStory("#pin-spacer-2", tl);
  }
  ...
  function buildSection3() {
    ...
    registerScrollStory("#pin-spacer-3", tl);
  }
  ...
  function buildSection4Reveal() {
    ...
    // Mbiyen: ScrollTrigger onUpdate, dipanggil KONTINU saben frame
    // progress berubah (dudu mung sepisan) sak suwene scrub. Supaya
    // perilaku persis padha, s4:reveal ing kene uga di-dispatch saben
    // updateScrollStory() nemokake progress > 0.12 — TANPA flag
    // sekali-saja — amarga pendengare (initCarousel() ing ngisor,
    // "window.addEventListener('s4:reveal', ...)") wis duwe guard-e
    // dhewe (`if (!autoplayId) startAutoplay()`), dadi aman lan ora
    // restart autoplay bola-bali senajan event iki metu bola-bali.
    registerScrollStory("#pin-spacer-4", tl, (progress) => {
      if (progress > 0.12) window.dispatchEvent(new CustomEvent("s4:reveal"));
    });
  }
  ...
  function buildSection6() {
    ...
    registerScrollStory("#pin-spacer-6", tl);
  }
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    window.addEventListener("load", () => {
      // Jaga-jaga: yen ana gambar/font sing mundur mlebune sawise kabeh
      // asset kelar dimuat lan nggawe reflow, posisi .pin-spacer-N iso
      // melu owah — itung ulang sepisan supaya progress tetep akurat.
      // Mbiyen iki ScrollTrigger.refresh(); saiki cukup updateScrollStory().
      if (window.updateScrollStory) {
        try { window.updateScrollStory(); } catch (err) { /* noop */ }
      }
    });
  });
})();